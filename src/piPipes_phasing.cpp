...
void read_file_into_unordered_map(const string &file_name,
                                  unordered_map<string, unordered_map<char, unordered_map<uint64_t, double>>> &umap)
{
  ...
  a.k.a, how many other loci, including itself, has the same sequence.
    ...
    try {
        opts.add_options()
            ...
            ("upper_limit,u",
             boost::program_options::value<int>(&upper_limit)->default_value(UPPERLIMIT),
                    ...
                    if (has_chr != umapB.end()) {
                        ...
                        if (has_strand != has_chr->second.end()) {
                            ...
                            if (has_pos != has_strand->second.end()) {
                                ...
                                *z_score += min(position.second, has_pos->second);
                            }
                        }
                        else
                            continue;
