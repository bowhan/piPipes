...
Please contact bo.han@Umassmed.edu for any questions.
    ...
    try {
        opts.add_options()
                ...
                ("upper_limit,u"
                 ...
                 , "the maximal overlap to calculate");
    ...
    for (int i = 0; i < num_of_threads; ++i) {
        if (file_a == file_b) {
            threads.emplace_back(PingPongPlayer<pptype>{poola, poola, ppt, z_score, &PingPong});
        } else {
            threads.emplace_back(PingPongPlayer<pptype>{poola, poolb, ppt, z_score, &PingPong});
        }
    }
