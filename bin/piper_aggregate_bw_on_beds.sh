...
then
	...
	do \
		...
		echo "bwtool agg ${EXT_LEN}:${EXT_LEN}:${EXT_LEN} -long-form=${t}meta,'Poisson P value,Fold Enriched,logLR' ${!t} $BIGWIG_FILES $OUTDIR/${t}.meta.txt && Rscript --slave ${PIPELINE_DIRECTORY}/bin/piper_draw_aggregate.R $OUTDIR/${t}.meta.txt  $PDF_DIR/${t}.meta ${t}.meta " >> $para_file
	done
