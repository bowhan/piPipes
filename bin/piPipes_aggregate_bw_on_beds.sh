...
then
	for t in ${TARGETS[@]}
	do \
		echo "bwtool agg ${EXT_LEN}:${EXT_LEN} -starts -long-form=${t}start,'Poisson P value,Fold Enriched,logLR' ${!t} $BIGWIG_FILES $OUTDIR/${t}.starts.txt && Rscript --slave ${PIPELINE_DIRECTORY}/bin/piPipes_draw_aggregate.R $OUTDIR/${t}.starts.txt $PDF_DIR/${t}.starts ${t}.TSS " >> $para_file
		echo "bwtool agg ${EXT_LEN}:${EXT_LEN} -ends -long-form=${t}end,'Poisson P value,Fold Enriched,logLR' ${!t} $BIGWIG_FILES $OUTDIR/${t}.ends.txt && Rscript --slave ${PIPELINE_DIRECTORY}/bin/piPipes_draw_aggregate.R $OUTDIR/${t}.ends.txt $PDF_DIR/${t}.ends ${t}.TES " >> $para_file
	#	echo "bwtool agg ${EXT_LEN}:${EXT_LEN}:${EXT_LEN} -meta-scale-all -long-form=${t}meta,'Poisson P value,Fold Enriched,logLR' ${!t} $BIGWIG_FILES $OUTDIR/${t}.meta.txt && Rscript --slave ${PIPELINE_DIRECTORY}/bin/piPipes_draw_aggregate.R $OUTDIR/${t}.meta.txt  $PDF_DIR/${t}.meta ${t}.meta " >> $para_file
