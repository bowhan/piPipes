...
[ ! -f .${JOBUID}.status.${STEP}.genome_bam_processing ] && \
	samtools view -bS ${GENOMIC_MAPPING_DIR}/${PREFIX}.x_rRNA.${GENOME}.Aligned.out.sam 2>/dev/null | tee ${GENOMIC_MAPPING_DIR}/${PREFIX}.x_rRNA.${GENOME}.Aligned.out.bam | bedtools_piPipes bamtobed -bedpe -mate1 -i - > ${GENOMIC_MAPPING_DIR}/${PREFIX}.x_rRNA.${GENOME}.bedpe && \
...
if [ "$GENOME" == "dm3" ]; then
	...
	echo2 "Making summary graph"
	[ ! -f .${JOBUID}.status.${STEP}.make_direct_mapping_sum_dm3_transposon ] && \
	grep -v 'NM_' $TRANSCRIPTOME_SIZES | grep -v 'NR_' > ${DIRECTMAPPING_DIR}/transposon.sizes && \
	bedtools_piPipes genomecov -i ${DIRECTMAPPING_DIR}/${PREFIX}.gene+transposon.sorted.unique.bed -g $TRANSCRIPTOME_SIZES -strand + -bg \
		> ${DIRECTMAPPING_DIR}/${PREFIX}.gene+transposon.sorted.unique.plus.bedGraph && \
	bedtools_piPipes genomecov -i ${DIRECTMAPPING_DIR}/${PREFIX}.gene+transposon.sorted.unique.bed -g $TRANSCRIPTOME_SIZES -strand - -bg \
		| awk 'BEGIN{FS=OFS="\t"}{$4=-$4;print $0}' \
	...
	ParaFly -c $paraFile -CPU $CPU && \
	paraFile=${OUTDIR}/drawFigures && \
	rm -rf ${DIRECTMAPPING_DIR}/${PREFIX}.gene+transposon.sorted.unique.bigWig.summary && \
	for i in `cut -f1 ${DIRECTMAPPING_DIR}/transposon.sizes`; do \
		[ ! -s ${DIRECTMAPPING_DIR}/${PREFIX}.gene+transposon.sorted.unique.plus.bigWig.$i  ] && awk -v binSize=${BINSIZE} 'BEGIN{for (i=0;i<binSize-1;++i){printf "%d\t", 0} print 0;}' > ${DIRECTMAPPING_DIR}/${PREFIX}.gene+transposon.sorted.unique.plus.bigWig.$i
		[ ! -s ${DIRECTMAPPING_DIR}/${PREFIX}.gene+transposon.sorted.unique.minus.bigWig.$i ] && awk -v binSize=${BINSIZE} 'BEGIN{for (i=0;i<binSize-1;++i){printf "%d\t", 0} print 0;}' > ${DIRECTMAPPING_DIR}/${PREFIX}.gene+transposon.sorted.unique.minus.bigWig.$i
		awk -v name=$i '{for (i=1;i<=NF;++i){printf "%s\t%d\t%d\n", name, i, $i}}' ${DIRECTMAPPING_DIR}/${PREFIX}.gene+transposon.sorted.unique.plus.bigWig.$i  > ${DIRECTMAPPING_DIR}/${PREFIX}.gene+transposon.sorted.unique.plus.bigWig.$i.t
		awk -v name=$i '{for (i=1;i<=NF;++i){printf "%s\t%d\t%d\n", name, i, $i}}' ${DIRECTMAPPING_DIR}/${PREFIX}.gene+transposon.sorted.unique.minus.bigWig.$i > ${DIRECTMAPPING_DIR}/${PREFIX}.gene+transposon.sorted.unique.minus.bigWig.$i.t
		paste ${DIRECTMAPPING_DIR}/${PREFIX}.gene+transposon.sorted.unique.plus.bigWig.$i.t ${DIRECTMAPPING_DIR}/${PREFIX}.gene+transposon.sorted.unique.minus.bigWig.$i.t | cut -f1,2,3,6 >> ${DIRECTMAPPING_DIR}/${PREFIX}.gene+transposon.sorted.unique.bigWig.summary
		rm -rf ${DIRECTMAPPING_DIR}/${PREFIX}.gene+transposon.sorted.unique.plus.bigWig.$i ${DIRECTMAPPING_DIR}/${PREFIX}.gene+transposon.sorted.unique.plus.bigWig.$i.t ${DIRECTMAPPING_DIR}/${PREFIX}.gene+transposon.sorted.unique.minus.bigWig.$i ${DIRECTMAPPING_DIR}/${PREFIX}.gene+transposon.sorted.unique.minus.bigWig.$i.t
	done && \
	Rscript --slave ${PIPELINE_DIRECTORY}/bin/piPipes_draw_summary.R ${DIRECTMAPPING_DIR}/${PREFIX}.gene+transposon.sorted.unique.bigWig.summary ${DIRECTMAPPING_DIR}/${PREFIX}.gene+transposon.sorted.unique.bigWig.summary $CPU $NormScale 1>&2 && \
	PDFs=${DIRECTMAPPING_DIR}/${PREFIX}.gene+transposon.sorted*pdf && \
	gs -q -dNOPAUSE -dBATCH -sDEVICE=pdfwrite -sOutputFile=$PDF_DIR/${PREFIX}.gene+transposon.sorted.unique.pdf ${PDFs} && \
	rm -rf ${PDFs} && \
	touch .${JOBUID}.status.${STEP}.make_direct_mapping_sum_dm3_transposon

	[ ! -f .${JOBUID}.status.${STEP}.eXpress_quantification_dm3_transposon ] && \
	express \
		-B $eXpressBATCH \
		-o $DIRECTMAPPING_DIR \
		--no-update-check \
		--library-size ${MapMass%.*} \
		$COMMON_FOLDER/${GENOME}.gene+transposon.fa \
		${DIRECTMAPPING_DIR}/${PREFIX}.gene+transposon.bam \
		1>&2 2> $DIRECTMAPPING_DIR/${PREFIX}.gene+transposon.eXpress.log && \
	awk -v depth=$NormScale 'BEGIN{OFS="\t"; getline; print}{$8*=depth; print}' $DIRECTMAPPING_DIR/results.xprs > \
		$DIRECTMAPPING_DIR/results.xprs.normalized && \
