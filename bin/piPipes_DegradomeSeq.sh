...
if [[ -n $PE_MODE ]]; then
	[ ! -f .${JOBUID}.status.${STEP}.generate_cleavage_strand_bowtie_index ] && \
		echo ">${PREFIX}.ext$RC_EXT.RC" > $INDEX_OUTDIR/${PREFIX}.r1.RC.ext${RC_EXT}.unique.fa && \
		awk 'BEGIN{OFS="\t"}{if ($6=="-"){$2=$3-1;} else {$3=$2+1;} print $0}' $BEDTOOLS_DIR/${PREFIX}.x_rRNA.${GENOME}.sorted.f0x40.noS.all.x_rpmk_MASK.bed12 | \
		bedtools slop -b $RC_EXT -i stdin -g $CHROM | \
		awk -v total_len=$((2*RC_EXT+1)) 'BEGIN{OFS="\t"}{ if ($3-$2==total_len) { $6=($6=="+"?"-":"+"); print $0} }' | \
		bedtools getfasta -fi $GENOME_FA -bed stdin -tab -s -fo /dev/stdout | cut -f2 | sort -u --temporary-directory=$INDEX_OUTDIR --parallel=$CPU >> $INDEX_OUTDIR/${PREFIX}.r1.RC.ext${RC_EXT}.unique.fa && \
		bowtie2-build $INDEX_OUTDIR/${PREFIX}.r1.RC.ext${RC_EXT}.unique.fa $INDEX_OUTDIR/${PREFIX}.r1.RC.ext${RC_EXT}.unique && \
		touch .${JOBUID}.status.${STEP}.generate_cleavage_strand_bowtie_index
	STEP=$((STEP+1))
else
	[ ! -f .${JOBUID}.status.${STEP}.generate_cleavage_strand_bowtie_index ] && \
		echo ">${PREFIX}.ext$RC_EXT.RC" > $INDEX_OUTDIR/${PREFIX}.RC.ext${RC_EXT}.unique.fa && \
		awk 'BEGIN{OFS="\t"}{if ($6=="-"){$2=$3-1;} else {$3=$2+1;} print $0}' ${GENOMIC_MAPPING_DIR}/${PREFIX}.x_rRNA.${GENOME}.sorted.noS.all.bed12 | \
		bedtools slop -b $RC_EXT -i stdin -g $CHROM | \
		awk -v total_len=$((2*RC_EXT+1)) 'BEGIN{OFS="\t"}{ if ($3-$2==total_len) { $6=($6=="+"?"-":"+"); print $0} }' | \
		bedtools getfasta -fi $GENOME_FA -bed stdin -tab -s -fo /dev/stdout | cut -f2 | sort -u --temporary-directory=$INDEX_OUTDIR --parallel=$CPU >> $INDEX_OUTDIR/${PREFIX}.RC.ext${RC_EXT}.unique.fa && \
		bowtie2-build $INDEX_OUTDIR/${PREFIX}.RC.ext${RC_EXT}.unique.fa $INDEX_OUTDIR/${PREFIX}.RC.ext${RC_EXT}.unique && \
		touch .${JOBUID}.status.${STEP}.generate_cleavage_strand_bowtie_index
	STEP=$((STEP+1))
fi
	...
	if [[ -n $PE_MODE ]]; then
		...
		[ ! -f .${JOBUID}.status.${STEP}.map_smRNA_to_degRC_index_$SMRNA_ID ] && \
			...
			bowtie2 \
				...
				-f -p $CPU \
				-U - 2>$SMRNA_MAP_DIR/`basename $SRA_ALL_BED2`.piRNA_map_to.${PREFIX}.species.b2.log | \
			samtools view -bS - | \
			bedtools bamtobed -i | \
			awk -v len=$((2*RC_EXT+1)) '{if($6=="+") { a[$2%len]++; b[($3-1)%len]++} else {c[$2%len]++;d[($3-1)%len]++}}END{for (i=0;i<len;++i) printf "%d\t%d\t%d\t%d\t%d\n", i+1, a[i], b[i], c[i], d[i]}' \
				> $SMRNA_MAP_DIR/`basename $SRA_ALL_BED2`.piRNA_map_to.${PREFIX}.species && \
