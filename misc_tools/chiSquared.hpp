    ...
    void readFile ( std::ifstream* in ) {
      ...
      for (size_t i = 0; i < _names.size (); ++i) {
        ...
        for (uint32_t j = 0; j < _N; ++j) {
          ...
          results += std::pow ( (_data[j][i] - expected>0.0?(_data[j][i] - expected):(expected-_data[j][i])) - yets,2)/expected;
          
          expected = ( tableTotal - rowTotal) * _count[j] / tableTotal;
          results += std::pow ( (_count[j] - _data[j][i] - expected>0?(_count[j] - _data[j][i] - expected):(_data[j][i]+expected-_count[j])) - yets, 2)/expected;
        }
        std::cout << '\t' << results << '\n';
      }
    }
