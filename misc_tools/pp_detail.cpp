...
void read_file_into_unordered_map(const string& file_name, 
	unordered_map<string, unordered_map<char, unordered_map<uint64_t, double>>>* umap_five
	) {
...
this program calculate the ping-pong score for each genomic posisitons.
	...
	try {
			opts.add_options ()
				...
				("file_a,a", boost::program_options::value<string>(&file_a)->default_value("stdin"), "first file used for ping-pong in BED2 format")
				("file_b,b", boost::program_options::value<string>(&file_b)->default_value("stdout"), "second file used for ping-pong in BED2 format")
	...
	if(!ifs) {cerr << "error, cannot open chromosome size file " << chrom_file << endl; exit(1); }
	string temp_str;
	...
	while(getline(ifa,temp_str,'\n')){
		...
		if(vec_[5]=="+") {
			if(_left+distance  < chrom_size[chr])	_pp_score = _score * (*chrPos5endValue2)[chr]['-'][_left+distance];
			if(_right+distance+1 < chrom_size[chr])	_phasing_score = min(_score, (double)(*chrPos5endValue2)[chr]['+'][_right+1]);
		} else {
