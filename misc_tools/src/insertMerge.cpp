...
#include "../include/insertMerge.hpp"
...
int main(int argc, char **argv) {
    ...
    opts.add_options()
            ...
            ("output,o"
             , boost::program_options::value<string>(&outputFileName)->default_value(string("stdout"))
             , "output file")
            ("common,c"
             , boost::program_options::value<int>(&commonField)->default_value(1)
             , "commond field to be used as key for merging, 1-based")
            ("target,t"
             , boost::program_options::value<int>(&mergeField)->default_value(0)
             , "target field to be merged, 1-based")
            ("default,d"
             , boost::program_options::value<string>(&defaultValue)
             , "the default target value to be used if this key does not exist");
